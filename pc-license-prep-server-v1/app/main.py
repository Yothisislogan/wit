...
DB_PATH = os.environ.get("DB_PATH", str(ROOT_DIR / "pc_prep.db"))

app = FastAPI(title="P&C License Prep Academy API", version="1.0.0")
...
def get_progress(learner_id: str = "default"):
    with db() as conn:
        rows = conn.execute("select * from progress where learner_id=?", (learner_id,)).fetchall()
        mistake_count = conn.execute("select count(*) c from mistakes where learner_id=?", (learner_id,)).fetchone()["c"]
...
def save_progress(payload: ProgressIn):
    now = int(time.time())
    with db() as conn:
        conn.execute("insert into progress(learner_id,lesson_id,completed,confidence,note,saved,updated_at) values(?,?,?,?,?,?,?) on conflict(learner_id,lesson_id) do update set completed=excluded.completed, confidence=excluded.confidence, note=excluded.note, saved=excluded.saved, updated_at=excluded.updated_at", (payload.learner_id, payload.lesson_id, int(payload.completed), payload.confidence, payload.note, int(payload.saved), now))
    return {"ok": True}


@app.get("/api/questions")
def question_set(module_id: str | None = None, limit: int = 10):
    qs = [q for q in QUESTIONS if module_id is None or q["module_id"] == module_id]
    if not qs:
        qs = QUESTIONS[:]
    random.shuffle(qs)
...
def submit_quiz(payload: QuizSubmit):
    ...
    with db() as conn:
        for qid, selected in payload.answers.items():
            ...
            if not is_correct:
                conn.execute("insert into mistakes(learner_id,question_id,selected,correct,created_at) values(?,?,?,?,?)", (payload.learner_id, qid, int(selected), int(q["answer"]), now))
            results.append({"id": qid, "correct": is_correct, "selected": selected, "answer": q["answer"], "explanation": q["explanation"]})
    total = max(len(results), 1)
    return {"score": round(correct / total * 100), "correct": correct, "total": total, "results": results}


@app.get("/api/mistakes")
def mistakes(learner_id: str = "default"):
    by_id = {q["id"]: q for q in QUESTIONS}
    with db() as conn:
        rows = conn.execute("select * from mistakes where learner_id=? order by created_at desc limit 50", (learner_id,)).fetchall()
    ...
    for m in MODULES:
        for lesson in m["lessons"]:
            for term in lesson.get("terms", []):
                ...
                if key not in seen:
                    seen.add(key)
                    cards.append({"term": term, "definition": f"Review this term in the lesson: {lesson['title']}", "module": m["title"]})
    return cards


@app.get("/api/study-plan")
def study_plan(learner_id: str = "default"):
...
def tutor(payload: TutorIn):
    ...
    return {"answer": "Coverage Coach: " + " ".join(matches[:2]), "guardrail": "General exam prep only. No state-specific legal or coverage advice."}
